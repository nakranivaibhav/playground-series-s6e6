"""node_0009 — draft (nn): TabM via the OFFICIAL `tabm` library (not hand-rolled).

Per CLAUDE.md rule 8 (libraries first): the model is the reference TabM (ICLR 2025,
parameter-efficient deep ensemble, k=32 members) from the `tabm` package, with
target-aware PiecewiseLinearEmbeddings from `rtdl_num_embeddings`. Only the thin
training loop is ours.

Why TabM after node_0008: a plain MLP was the most de-correlated arm but too weak solo
(0.9550) to lift the blend. TabM is a much stronger tabular NN — if it reaches near-GBDT
solo accuracy while staying de-correlated, it can finally help the combine.
...
fm = train_model(apply_std(Xnum_all, mu, sd), Xc_all, y, cat_cards, class_w)
...
model: TabM (library tabm=={tabm.__version__}, k=32 ensemble) + PiecewiseLinearEmbeddings (rtdl), CUDA
