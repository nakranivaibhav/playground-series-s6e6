"""node_0023 — CatBoost undertraining fix: raise iterations with proper early stopping.

Built on: node_0003 (CatBoost, native cats, fs_colors). Inherits byte-identical
fold loop, OOF structure, feature engineering (cast_categoricals, add_color_features,
feature_columns from clean.py), shuffled-label control, submission logic, and the
fs_colors feature set. Do NOT swap features — the full-feature CatBoost already exists
as node_0012 so this keeps fs_colors to stay de-correlated.

ONE atomic change vs node_0003: fix undertraining by raising iterations to 5000 and
increasing od_wait to 100 (was 800 iterations / od_wait=50). CatBoost's use_best_model
with eval_set on the fold's own val split is standard fold-honest early-stopping —
the val rows are used only for stopping, they contribute to no other fold's OOF.
Everything else (learning_rate, depth, auto_class_weights, native cat handling,
seed, thread_count) stays byte-identical to node_0003.
...
# ---------------------------------------------------------------------------
NODE_SRC = Path(__file__).resolve().parent           # …/node_0023/src
NODE_DIR = NODE_SRC.parent                           # …/node_0023/
...
# metrics.md
# ---------------------------------------------------------------------------
metrics_text = f"""# node_0023 metrics
