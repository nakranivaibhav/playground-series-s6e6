"""node_0024 — improve (nn): RealMLP-HPO strong recipe.

Built on: node_0021 (RealMLP base via pytabkit). The fs_research feature matrix
(22 continuous + 4 flag + 2 categorical), frozen folds.json loop, fold-honest
OOF + test_probs interface, and leakage discipline are byte-identical to node_0021.

THE ONE ATOMIC CHANGE: replace the bare `RealMLP_TD_Classifier` with a strengthened
recipe using `RealMLP_TD_Classifier` with raised capacity: n_epochs=512 (577k rows
under-fit at the default 256), hidden sizes [512,512,512] (wider/deeper), PLR
piecewise-linear numerical embeddings ON, batch_size=8192 for throughput.
val_metric_name="1-balanced_accuracy" so the model uses its own internal early-stop
on the fold's held split — standard, not leakage.

We use the strengthened TD variant (not HPO) because per-dataset HPO on 577k rows
x 5 folds would be prohibitively slow (hours per fold). The TD strengthened recipe
is the most impactful single change and keeps wall-clock manageable.
...
Leakage discipline (fit_in_fold):
  ...
  - RealMLP_TD_Classifier is instantiated fresh and .fit() called INSIDE each fold on
    train rows only. Internal early-stop uses the fold's held split (fold-honest).
...
print("  pytabkit imported OK — using RealMLP_TD_strong variant", flush=True)
...
def make_model():
    """Fresh RealMLP_TD_Classifier with strengthened capacity (the ONE atomic change)."""
    return RealMLP_TD_Classifier(
        device="cuda",
        random_state=SEED,
        n_epochs=512,           # up from 256 — 577k rows under-fit at default
        hidden_sizes=[512, 512, 512],   # wider/deeper than TD default
        use_plr_embeddings=True,        # PLR piecewise-linear numerical embeddings ON
        batch_size=8192,        # larger batch for throughput on big data
...
per_fold = []
print("Running OOF (RealMLP_TD_strong recipe, pytabkit, CUDA) …", flush=True)
