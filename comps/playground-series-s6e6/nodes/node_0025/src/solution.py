"""node_0025 — improve (nn): TabPFN-2.5 @ v2.5_large-samples checkpoint, 50k context, K=8 subsamples.

Built on: node_0022 (TabPFN subsample-ensemble, K=8 subsamples x 10k context). The
fs_research feature load, frozen folds.json loop, fold-honest OOF/test interface,
stratified_subsample helper, and the entire data-engineering pipeline are kept
byte-identical. Only the following changes:

Change: ONE atomic upgrade — use the TabPFN-2.5 v2.5_large-samples checkpoint
(Prior-Labs/tabpfn_2_5, file tabpfn-v2.5-classifier-v2.5_large-samples.ckpt).
This checkpoint is specifically tuned for high-row-count data. Configuration:
  1. K_SUBSAMPLES = 8 (bagging saturates fast; coverage is 8x50k = 400k).
  2. SUBSAMPLE_SIZE = 50_000 (5x richer context than node_0022's 10k).
  3. model_path = the v2.5_large-samples .ckpt file in ~/.cache/tabpfn/.
...
# TabPFN-2.5 large-samples checkpoint path
TABPFN_CKPT = Path.home() / ".cache" / "tabpfn" / "tabpfn-v2.5-classifier-v2.5_large-samples.ckpt"
...
print(f"Device: {DEVICE} | TabPFN-2.5 checkpoint: {TABPFN_CKPT}", flush=True)
assert TABPFN_CKPT.exists(), (
    f"TabPFN-2.5 large-samples checkpoint not found: {TABPFN_CKPT}\n"
    "Download it from HuggingFace: Prior-Labs/tabpfn_2_5 "
...
per_fold = []
print("Running OOF (TabPFN-2.5 large-samples, 50k context, K=8) ...", flush=True)
