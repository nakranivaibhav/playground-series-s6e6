"""node_0030 — draft (gbdt): LightGBM on fs_realmlp_fe features.

THE ONE ATOMIC CHANGE vs node_0028:
  Replace the hand-rolled RealMLP model with a well-tuned LightGBM classifier.
  The ENTIRE feature-engineering pipeline (stateless FE + fit-in-fold
  TargetEncoder/KBins) is kept byte-identical from node_0028.

LightGBM config:
  - NO native categorical handling (integer codes passed as numeric to avoid
    LightGBM's expensive categorical-split search on high-cardinality features).
    spectral_type (4 vals) and galaxy_population (2 vals) are already well-ordered
    by factorize; LightGBM threshold splits work fine.
  - class_weight="balanced" for Balanced Accuracy (macro recall)
  - n_estimators up to 2000, learning_rate=0.05, num_leaves=127
  - early stopping on fold val (150 rounds)
  - CPU mode, n_jobs=-1

Leakage discipline (preserved from node_0028):
...
Note: NumericalPreprocessor (median/IQR scaling) NOT needed — LightGBM is
scale-invariant.
