"""node_0034 — CatBoost on fs_realmlp_fe features (CPU).

THE ONE ATOMIC CHANGE vs node_0028:
  Replace the hand-rolled RealMLP with a well-tuned CatBoost classifier on
  the SAME fs_realmlp_fe feature-set. Native categorical handling, balanced
  class weights (auto_class_weights='Balanced'), iterations up to 5000 with
  early-stopping (od_wait=100) on each fold's own val split. CPU only
  (task_type='CPU') to avoid GPU contention with concurrent TabM node.

Feature-set fs_realmlp_fe (unchanged from node_0028):
...
def make_catboost(seed: int = SEED, iterations: int = 5000) -> CatBoostClassifier:
    """Return a fresh, well-tuned CatBoostClassifier for fs_realmlp_fe features."""
    return CatBoostClassifier(
        ...
        use_best_model=True,
        verbose=False,
        # Border count for float features — higher = better splits (default 254)
        border_count=254,
    )
...
for fi in folds_list:
    ...
    del model, train_pool, val_pool, test_pool, X_tr_fold, X_val_fold, X_te_fold
    gc.collect()
