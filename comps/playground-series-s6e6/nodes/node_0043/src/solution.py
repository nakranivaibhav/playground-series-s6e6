"""node_0043 — CatBoost config-B (de-correlated hyperparameters) on fs_realmlp_fe features (CPU).

THE ONE ATOMIC CHANGE vs node_0039:
  Swap to a DIFFERENT CatBoost hyperparameter configuration for de-correlation.
  node_0039 used: depth=6, border_count=128, learning_rate=0.05, l2_leaf_reg=3.0,
                  grow_policy=default (SymmetricTree).
  node_0043 uses: depth=7, border_count=96, learning_rate=0.03, iterations=8000,
                  l2_leaf_reg=5.0, grow_policy=Lossguide.
  Memory-safe: depth=7/border_count=96 keeps peak RSS well under 20 GB.
  (n34 OOM'd at 29.5 GB with depth=8/border_count=254; n39 peaked at 3.3 GB
   with depth=6/border_count=128; depth=7/border_count=96 is memory-intermediate.)
  Everything else (FE, native cats, balanced class weights, early-stopping,
  OOF scaffold, gc.collect between folds) is byte-identical to node_0039.
...
def make_catboost(seed: int = SEED, iterations: int = 8000) -> CatBoostClassifier:
    """Return config-B CatBoostClassifier — distinct hyperparameters for de-correlation from node_0039.

    node_0039: depth=6, border_count=128, lr=0.05, l2_leaf_reg=3.0, grow_policy=SymmetricTree
    node_0043: depth=7, border_count=96,  lr=0.03, l2_leaf_reg=5.0, grow_policy=Lossguide
    Memory-safe: peak RSS well under 20 GB (border_count=96 < n39's 128, depth=7 ≤ safe limit).
    Lower lr + more iterations allows finer convergence; Lossguide makes tree-building
    asymmetric, decorrelating the learned structure from n39's SymmetricTree.
    """
    return CatBoostClassifier(
        iterations=iterations,
        learning_rate=0.03,       # Lower LR than n39 (0.05) → finer convergence with more iterations
        depth=7,                  # One more than n39 (6) → richer trees, different splits
        l2_leaf_reg=5.0,          # Stronger regularization than n39 (3.0)
        grow_policy="Lossguide",  # Asymmetric tree growth — decorrelates from n39's SymmetricTree
        loss_function="MultiClass",
        eval_metric="Accuracy",   # Accuracy proxy for early stopping; BA computed manually
        auto_class_weights="Balanced",
        random_seed=seed,
        task_type="CPU",
        thread_count=8,
        od_type="Iter",
        od_wait=150,              # More patience for lower LR
        use_best_model=True,
        verbose=False,
        border_count=96,          # Less than n39 (128) — memory-safe at depth=7
    )
