"""node_0045 — improve (nn): RealMLP reference recipe + original-SDSS17 prior features.

Built on: node_0028 (RealMLP-reference recipe, cv 0.969065). Everything byte-identical
except ONE change: add original-SDSS17 prior features (fs_origprior) to the input matrix.

THE ONE ATOMIC CHANGE:
  Add P(class|color-bin-key) prior features computed on data/sdss17/star_classification.csv.
  Orig rows NOT appended to training (USE_ORIGINAL_ROWS=False). Priors computed on orig only
  (orig labels are disjoint from our train/val — leak-clean). Same bin keys as node_0044's
  fs_zoo.py (art_u_g_half, art_g_r_half, art_redshift_tenth, art_u_floor_x_z_floor, etc.).
  Prior columns are concatenated to fs_realmlp_fe matrix, then scaled by same robust
  preprocessing (median_center -> robust_scale) fit in-fold.

node_0028 original docstring follows below for reference.

THE ONE ATOMIC CHANGE (two parts):
(a) STATELESS FE (feature-set fs_realmlp_fe):
    - redshift ratios: g/redshift, i/redshift
    - log1p(shifted_redshift)
    - 7 color pairs: u-g, g-r, r-i, i-z, u-r, g-i, r-z
    - mag_mean, mag_range over u/g/r/i/z
    - integer-floor categorical views of every base numeric
    - category cross-combos (alpha_cat_, delta_cat_), (u_cat_, z_cat_)
    - delta quantile bins (100 and 500 bins) -- fit_in_fold via KBinsDiscretizer
    - target encoding on combo cats (fit_in_fold inside each fold)
    - All stateless parts computed once; fit_in_fold parts (KBins, TargetEnc,
      NumericalPreprocessor) fit on train-fold rows only.

(b) Hand-rolled RealMLP (matching the reference architecture):
    - n_ens=8 internal ensemble, hidden [512,512,512]
    - PBLD periodic numerical embeddings (freq_scale=2.33, hidden_dim=16, out_dim=5)
    - ScalingLayer front, expm4t dropout schedule
    - GELU + per-parameter-group optimizer (AdamW)
    - preprocessing: median_center -> robust_scale(IQR) (fit on train fold only)
    - EMA decay=0.9979, flat_cos lr schedule, loss_prior_power=1.075, 6 epochs

WHY hand-rolled (CLAUDE.md rule 8 note):
  bare pytabkit-TD on bare features scored 0.949 (node_0024), far below the
  reference. The pytabkit API does not expose loss_prior_power,
  EMA, or per-group lr schedules at the level needed to replicate the recipe
  faithfully. The reference code hand-rolls all components; we port them here.
...
Leakage discipline:
  ...
  - TargetEncoder: fit on train-fold only (using sklearn's internal CV=5 strategy),
    applied to val and test.
  - NumericalPreprocessor (median/IQR): fit on X_tr_num inside each fold.
  - frozen folds.json used throughout; no refitting of folds.
  - Orig-prior features: P(class|key) computed on sdss17/star_classification.csv only.
    No train/val/test labels used. Leak-clean.
...
# ─── Original-prior feature helpers ──────────────────────────────────────────

ORIG_BIN_SPECS = [
    # (col_to_bin, scale, tag_suffix)
    ("u_g", 2.0, "half"),
    ("g_r", 2.0, "half"),
    ("redshift", 10.0, "tenth"),
]

ORIG_FLOOR_COLS = ["u", "g", "r", "i", "z", "alpha", "redshift"]

ORIG_CROSS_COMBOS = [
    ("art_u_g_half", "art_redshift_tenth"),
    ("art_g_r_half", "art_redshift_tenth"),
    ("art_u_floor", "art_z_floor"),
]


def _build_art_keys(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build the same artifact bin-key columns used by fs_zoo.py for orig-prior look-ups.
    Stateless / row-wise — safe on any partition.
    """
    out = df.copy()

    # Color pairs needed for bins
    for a, b in [("u", "g"), ("g", "r")]:
        col = f"{a}_{b}"
        if col not in out.columns:
            out[col] = (out[a] - out[b]).astype("float32")

    # Artifact scaled bins
    for c, scale, tag in ORIG_BIN_SPECS:
        ...
        out[f"art_{c}_{tag}"] = binned.astype(str)

    # Floor-based keys
    for c in ORIG_FLOOR_COLS:
        vals = pd.to_numeric(out[c], errors="coerce").values.astype(np.float32)
        finite = ~np.isnan(vals)
        floored = np.floor(np.where(finite, vals, 0.0)).astype(np.int32)
        floored = np.where(finite, floored, -2147483648)
        out[f"art_{c}_floor"] = floored.astype(str)

    # Cross combos
    for a, b in ORIG_CROSS_COMBOS:
...
def _compute_orig_priors(orig_df: pd.DataFrame, orig_y: np.ndarray) -> dict:
    """
    Compute P(class|key) tables from orig data only.
    Returns dict: col_name -> {category_key -> array of 3 probs}
    Computed ONCE before the fold loop. Leak-clean: no train/val/test labels used.
    """
    counts_per_class = np.bincount(orig_y.astype(np.int32), minlength=N_CLASSES).astype(np.float32)
    prior = counts_per_class / max(counts_per_class.sum(), 1.0)

    orig_key_cols = (
        [f"art_{c}_{tag}" for c, _, tag in ORIG_BIN_SPECS] +
        [f"art_{c}_floor" for c in ORIG_FLOOR_COLS] +
        [f"art_{a}_x_{b}" for a, b in ORIG_CROSS_COMBOS if f"art_{a}_x_{b}" in orig_df.columns]
    )

    tables = {}
    y_arr = orig_y.astype(np.int32)
    for col in orig_key_cols:
        if col not in orig_df.columns:
            continue
        keys = orig_df[col].astype(str)
        tmp = pd.DataFrame({"key": keys, "y": y_arr})
        rate_tables = []
        for cls_idx in range(N_CLASSES):
            hit = (tmp["y"] == cls_idx).astype(np.float32)
            rates = tmp.assign(hit=hit).groupby("key")["hit"].mean()
            rate_tables.append(rates)
        tables[col] = (rate_tables, prior)
    return tables


def add_orig_prior_cols(df: pd.DataFrame, prior_tables: dict) -> list:
    """
    Add orig_*_prior_CLASS columns to df in-place. Returns list of new column names.
    """
    added = []
    for col, (rate_tables, prior_arr) in prior_tables.items():
        # Build key series for this df
        key = df[col].astype(str) if col in df.columns else None
        if key is None:
            continue
        for cls_idx in range(N_CLASSES):
            cls_name = CLASSES[cls_idx]
            col_name = f"orig_{col}_prior_{cls_name}"
            df[col_name] = key.map(rate_tables[cls_idx]).fillna(float(prior_arr[cls_idx])).astype("float32")
...
# ─── Load orig SDSS17 data + compute prior tables (ONCE, leak-clean) ──────────
log("Loading orig SDSS17 data for prior computation …")
ORIG_PATH = COMP_DIR / "data/sdss17/star_classification.csv"
orig_raw = pd.read_csv(ORIG_PATH)
log(f"  orig_raw={orig_raw.shape}")

# Drop -9999 placeholder rows in u/g/z bands; keep GALAXY/QSO/STAR only
...
# Map orig labels  (use same LABEL_MAP as train)
orig_y_arr = orig_raw["class"].map(LABEL_MAP).astype(int).values

# Build artifact bin keys for orig data
orig_fe = _build_art_keys(orig_raw)
log(f"  built art keys for orig")

# Compute prior tables (on orig only — no train/val labels)
PRIOR_TABLES = _compute_orig_priors(orig_fe, orig_y_arr)
log(f"  prior_tables: {len(PRIOR_TABLES)} key columns")
...
# Build artifact bin keys for train + test (stateless, row-wise, no fitting)
X_stateless = _build_art_keys(X_stateless)
X_test_stateless = _build_art_keys(X_test_stateless)

# Add orig-prior columns (lookup from PRIOR_TABLES — stateless after the tables are built)
_orig_prior_cols_tr = add_orig_prior_cols(X_stateless, PRIOR_TABLES)
_orig_prior_cols_te = add_orig_prior_cols(X_test_stateless, PRIOR_TABLES)
_orig_prior_numeric_cols = sorted(set(_orig_prior_cols_tr) | set(_orig_prior_cols_te))
log(f"  added {len(_orig_prior_numeric_cols)} orig-prior numeric columns")

# Drop the art key columns (string type) and any intermediate float color columns
# that were only needed for prior lookup. Keep only orig_*_prior_* float columns.
_cols_to_drop_from_stateless = (
    [c for c in X_stateless.columns if c.startswith("art_") and X_stateless[c].dtype == object] +
    ["u_g", "g_r"]  # intermediate color cols added by _build_art_keys (already have _u-g, _g-r)
)
X_stateless = X_stateless.drop(columns=_cols_to_drop_from_stateless, errors="ignore")
X_test_stateless = X_test_stateless.drop(columns=_cols_to_drop_from_stateless, errors="ignore")
log(f"  dropped intermediate art key / color columns; kept orig-prior float cols")
