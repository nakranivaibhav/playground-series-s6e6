"""node_0050 — symmetric one-vs-rest 3-binary heads on fs_realmlp_fe.

THE ONE ATOMIC CHANGE vs node_0030 (and the controlled A/B vs node_0049):
  Replace the single 3-class LightGBM head with THREE INDEPENDENT one-vs-rest
  binary heads, each built ENTIRELY INSIDE each frozen fold:
    Head G: GALAXY  vs rest → pG = P(GALAXY)
    Head Q: QSO     vs rest → pQ = P(QSO)
    Head S: STAR    vs rest → pS = P(STAR)
  Each head = LGBM+XGB blend 0.60*LGBM + 0.40*XGB (same hyperparams as RECIPE.md).
  COMPOSE per row: raw=[pG,pQ,pS]; P = raw / raw.sum(axis=1, keepdims); clip [1e-7,1].
  OOF on held-out folds; TEST probs = mean of per-fold composed test predictions.
  ...
  Also reports:
    ...
    3) CORE15 + node_0049 + node_0050 (both chain framings) as 17th-base stack.

Leakage discipline (verified for symmetric OvR):
  ...
  - Each OvR head: fit on FULL train-fold (binary label for that class-vs-rest).
    Inner ES val carved from TRAIN-FOLD only (never from the held-out OOF fold).
  - Each head is independent; no conditioning on other heads — fully symmetric.
...
Outputs:
  ...
  features.txt                 features used by head G model
"""
...
# ─── Feature engineering (byte-identical to node_0030 / node_0049) ──────────
...
def fit_binary_blend_single(X_tr, y_tr, X_val_es, y_val_es, X_query, fold_seed):
    """
    Fit LGBM+XGB blend on (X_tr, y_tr) with early-stop on (X_val_es, y_val_es).
    Returns (p_query, best_lgbm, best_xgb) — P(positive class).
    ...
    pL = lgbm.predict_proba(X_query)[:, 1].astype("float32")
    ...
    pX = xgb.predict_proba(X_query)[:, 1].astype("float32")
    best_xgb = xgb.best_iteration
    del xgb; gc.collect()

    p = 0.60 * pL + 0.40 * pX
    return p, best_lgbm, best_xgb


def compose_ovr(pG, pQ, pS):
    """Compose three independent OvR binary scores to 3-class probs by normalization."""
    raw = np.stack([pG, pQ, pS], axis=1).astype("float32")
...
log("Starting SYMMETRIC ONE-VS-REST OOF loop ...")
...
for fi in folds_list:
    ...
    # Use GALAXY-vs-rest as the TargetEncoder binary target (arbitrary but consistent)
    y_tr_for_te = (y_tr == GALAXY_IDX).astype(int)
    ...
    X_tr_fold, X_val_fold, X_te_fold, te_names = add_target_encoding_binary(
        X_tr_fold, y_tr_for_te, X_val_fold, X_te_fold, combo_names, fold_seed
    )
    ...
    # Build one ES val set per head but same indices (different binary labels)
    # Predict both val and test in one call per head — no separate refit

    # ── Head G: GALAXY vs rest ────────────────────────────────────────────────
    y_G_fit = (y_tr[fit_local]  == GALAXY_IDX).astype(int)
    y_G_es  = (y_tr[es_local]   == GALAXY_IDX).astype(int)
    # Concatenate val+test for a single predict call (saves overhead)
    n_val = len(val_idx)
    X_query_G = pd.concat([X_val_fold, X_te_fold], ignore_index=True)

    log(f"  Head-G (GALAXY vs rest): fit n={len(fit_local)}, es n={len(es_local)}")
    pG_all, b_lgbm_G, b_xgb_G = fit_binary_blend_single(
        X_fit, y_G_fit, X_es, y_G_es, X_query_G, fold_seed + 0
    )
    pG_val  = pG_all[:n_val]
    pG_test = pG_all[n_val:]
    log(f"  Head-G best iters: lgbm={b_lgbm_G}  xgb={b_xgb_G}")

    # ── Head Q: QSO vs rest ───────────────────────────────────────────────────
    y_Q_fit = (y_tr[fit_local]  == QSO_IDX).astype(int)
    y_Q_es  = (y_tr[es_local]   == QSO_IDX).astype(int)
    X_query_Q = pd.concat([X_val_fold, X_te_fold], ignore_index=True)

    log(f"  Head-Q (QSO vs rest): fit n={len(fit_local)}, es n={len(es_local)}")
    pQ_all, b_lgbm_Q, b_xgb_Q = fit_binary_blend_single(
        X_fit, y_Q_fit, X_es, y_Q_es, X_query_Q, fold_seed + 1
    )
    pQ_val  = pQ_all[:n_val]
    pQ_test = pQ_all[n_val:]
    log(f"  Head-Q best iters: lgbm={b_lgbm_Q}  xgb={b_xgb_Q}")

    # ── Head S: STAR vs rest ──────────────────────────────────────────────────
    y_S_fit = (y_tr[fit_local]  == STAR_IDX).astype(int)
    y_S_es  = (y_tr[es_local]   == STAR_IDX).astype(int)
    X_query_S = pd.concat([X_val_fold, X_te_fold], ignore_index=True)

    log(f"  Head-S (STAR vs rest): fit n={len(fit_local)}, es n={len(es_local)}")
    pS_all, b_lgbm_S, b_xgb_S = fit_binary_blend_single(
        X_fit, y_S_fit, X_es, y_S_es, X_query_S, fold_seed + 2
    )
    pS_val  = pS_all[:n_val]
    pS_test = pS_all[n_val:]
    log(f"  Head-S best iters: lgbm={b_lgbm_S}  xgb={b_xgb_S}")

    # ── Compose 3-class probs (symmetric normalize) ───────────────────────────
    val_proba  = compose_ovr(pG_val,  pQ_val,  pS_val)
    test_proba = compose_ovr(pG_test, pQ_test, pS_test)
    ...
    del X_tr_fold, X_val_fold, X_te_fold, X_fit, X_es, X_query_G, X_query_Q, X_query_S
    gc.collect()
...
# ─── Re-stack A/B: CORE15 + node_0050 as 16th base ──────────────────────────
...
node50_oof_log  = logp(oof_proba)
node50_test_log = logp(test_proba_accum)

OOF_16  = np.concatenate([OOF_CORE,  node50_oof_log],  axis=1)
TEST_16 = np.concatenate([TEST_CORE, node50_test_log], axis=1)
log(f"  stacked OOF={OOF_16.shape}  ({len(BASES)+1} bases)")

fval = [np.asarray(f["val_idx"]) for f in folds_list]

stack_oof_16 = np.zeros((n_train, NC), dtype=np.float32)
for vi in fval:
    tr_s = np.setdiff1d(np.arange(n_train), vi)
    m    = fit_meta(OOF_16[tr_s], y_all[tr_s])
    stack_oof_16[vi] = m.predict_proba(OOF_16[vi])

restack_16_folds = []
for i, vi in enumerate(fval):
    other = np.setdiff1d(np.arange(n_train), vi)
    w    = best_thr_de(stack_oof_16[other], y_all[other])
    pred = np.argmax(stack_oof_16[vi] * w, axis=1)
    s    = score_fn(y_all[vi], pred)
    restack_16_folds.append(s)
    log(f"  re-stack16 fold {i}: score={s:.6f}  w=[{w[0]:.4f},{w[1]:.4f},{w[2]:.4f}]")
    print(f"restack16_fold{i}={s:.6f}", flush=True)

rs16_mean = float(np.mean(restack_16_folds))
rs16_sem  = float(np.std(restack_16_folds, ddof=1) / np.sqrt(len(restack_16_folds)))
log(f"RE-STACK CORE15+n50 cv={rs16_mean:.6f}+/-{rs16_sem:.6f}  (champ 0.969808)")
print(f"restack16_cv={rs16_mean:.6f}", flush=True)

# ─── Triple stack: CORE15 + node_0049 + node_0050 (17 bases) ─────────────────
node49_oof_path  = nodes_dir / "node_0049" / "oof.npy"
node49_test_path = nodes_dir / "node_0049" / "test_probs.npy"

if node49_oof_path.exists() and node49_test_path.exists():
    log("Building CORE15 + node_0049 + node_0050 triple stack (17 bases) ...")
    node49_oof_log  = logp(np.load(node49_oof_path))
    node49_test_log = logp(np.load(node49_test_path))

    OOF_17  = np.concatenate([OOF_CORE, node49_oof_log, node50_oof_log],  axis=1)
    TEST_17 = np.concatenate([TEST_CORE, node49_test_log, node50_test_log], axis=1)
    log(f"  triple-stacked OOF={OOF_17.shape}  (17 bases)")

    stack_oof_17 = np.zeros((n_train, NC), dtype=np.float32)
    for vi in fval:
        tr_s = np.setdiff1d(np.arange(n_train), vi)
        m    = fit_meta(OOF_17[tr_s], y_all[tr_s])
        stack_oof_17[vi] = m.predict_proba(OOF_17[vi])

    restack_17_folds = []
    for i, vi in enumerate(fval):
        other = np.setdiff1d(np.arange(n_train), vi)
        w    = best_thr_de(stack_oof_17[other], y_all[other])
        pred = np.argmax(stack_oof_17[vi] * w, axis=1)
        s    = score_fn(y_all[vi], pred)
        restack_17_folds.append(s)
        log(f"  re-stack17 fold {i}: score={s:.6f}  w=[{w[0]:.4f},{w[1]:.4f},{w[2]:.4f}]")
        print(f"restack17_fold{i}={s:.6f}", flush=True)

    rs17_mean = float(np.mean(restack_17_folds))
    rs17_sem  = float(np.std(restack_17_folds, ddof=1) / np.sqrt(len(restack_17_folds)))
    log(f"RE-STACK CORE15+n49+n50 cv={rs17_mean:.6f}+/-{rs17_sem:.6f}  (champ 0.969808)")
    print(f"restack17_cv={rs17_mean:.6f}", flush=True)
else:
    log("node_0049 OOF not found — skipping triple stack")
    print("restack17_cv=N/A", flush=True)
