"""node_0028 — draft (nn): RealMLP reference recipe (FE + PBLD).

Built on: root (new draft, NOT an improve on node_0024).
Scaffolding adapted from node_0024 (frozen folds.json OOF loop, same output interface),
but FE and model are ENTIRELY replaced with the proven public reference recipe from
refs/realmlp-v5-for-s6e6.py (cdeotte) and cross-checked against
refs/ps-s6-e6-realmlp-pytorch.py (yekenot).
...
(b) Hand-rolled RealMLP (matching the reference architecture):
    ...
    - ScalingLayer front, expm4t dropout schedule (~0.044 base)
...
WHY hand-rolled (CLAUDE.md rule 8 note):
  ...
  reference's ~0.96973. The pytabkit API does not expose loss_prior_power,
...
SEED = 42
import os as _os
_VARIANT = _os.environ.get("C1_VARIANT","all")
_SUF = _os.environ.get("C1_SUF","_all")
_flagged = np.load("/tmp/c1_flagged.npy")
N_CLASSES = 3
...
# C1: build prune set per variant (GALAXY class index = 0)
if _VARIANT == "galaxy":
    _PRUNE_SET = _flagged[y_all[_flagged] == 0]
else:
    _PRUNE_SET = _flagged
log(f"C1 variant={_VARIANT} prune-set size={len(_PRUNE_SET)}")
...
for fi in folds_list:
    ...
    # C1 cleanlab PRUNE (drop flagged train rows; val rows untouched -> OOF comparable)
    _drop = _PRUNE_SET[np.isin(_PRUNE_SET, tr_idx)]
    tr_idx = np.setdiff1d(tr_idx, _drop)
    log(f"  C1 prune[{_VARIANT}]: dropped {len(_drop)} flagged train rows -> train={len(tr_idx)}")
...
# ─── Save OOF ────────────────────────────────────────────────────────────────
np.save(NODE_DIR / f"oof{_SUF}.npy", oof_proba)
log(f"Saved oof.npy shape={oof_proba.shape}")

# ─── Save test_probs ─────────────────────────────────────────────────────────
np.save(NODE_DIR / f"test_probs{_SUF}.npy", test_proba_accum)
...
sub.to_csv(NODE_DIR / f"submission{_SUF}.csv", index=False)
