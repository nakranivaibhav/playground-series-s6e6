...
def stateless_fe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # C5: provenance flags — exact-value membership of alpha/delta in the real SDSS17 catalog
    # (label-free: only feature VALUES read, never the orig class column). stateless.
    import pandas as _pd
    _o = _pd.read_csv(COMP_DIR / "data/sdss17/star_classification.csv", usecols=["alpha","delta"])
    _oa = set(np.round(_o["alpha"].values, 6)); _od = set(np.round(_o["delta"].values, 6))
    a_in = np.isin(np.round(df["alpha"].values, 6), list(_oa)).astype("float32")
    d_in = np.isin(np.round(df["delta"].values, 6), list(_od)).astype("float32")
    df["_alpha_in_orig"] = a_in
    df["_delta_in_orig"] = d_in
    df["_both_in_orig"] = (a_in * d_in).astype("float32")
