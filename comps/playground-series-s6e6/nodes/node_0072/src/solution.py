"""node_0072 — widen bank beyond n70's set.

Compose with n70: start from bank-17 + ft_transformer (node_0070's selected set, cv=0.970211).
ASSERT baseline reproduces 0.970211 (±0.0002) before forward-selecting new sources.

New candidates from refs/a4_refresh_2026-06-12:
  - stacking_0_97093    (oof_lr_stacker_v7.npy)
  - meta_stacker_97105  (oof_lr_stacker_v7.npy)
  - gpu_lr_v9           (oof_lr_stacker_v9.npy)

Greedy forward-select; stop when no candidate improves CV by > +0.00003.
"""
...
def main():
    ...
    # ---- Load public bank-17 (same manifest as node_0070 / champion) ----
    ...
    # ---- Also load ft_transformer (node_0070's selected candidate) ----
    ...
    ft_oof  = norm(load_ext_csv(ft_oof_path, n))
    ft_test = norm(load_ext_csv(ft_test_path, nt))
    ft_ba = balanced_accuracy_score(y, ft_oof.argmax(1))
    assert ft_oof.shape == (n, 3) and ft_test.shape == (nt, 3)
    assert ft_ba > 0.85, f"ft_transformer solo BA {ft_ba:.4f} — column order issue"
    print(f"\nft_transformer: solo_ba={ft_ba:.6f}  shape={ft_oof.shape}")

    # ---- Build baseline: bank-{len(good)} + ft_transformer (n70's set) ----
    bank_oof  = [logp(POOF[k]) for k in good] + [logp(ft_oof)]
    bank_test = [logp(PTEST[k]) for k in good] + [logp(ft_test)]
    n70_expected = 0.970211

    print(f"\n=== Step 1: Assert n70 baseline (bank-{len(good)}+ft_transformer, expected~{n70_expected}) ===")
    base_cv, base_sem, _ = eval_cols(bank_oof, y, fval)
    print(f"baseline cv={base_cv:.6f}  sem={base_sem:.6f}")
    diff = abs(base_cv - n70_expected)
    print(f"diff from n70 expected {n70_expected}: {diff:.6f}")
    if diff >= 0.0002:
        raise SystemExit(
            f"BASELINE ASSERTION FAILED: got {base_cv:.6f}, expected ~{n70_expected}, diff={diff:.6f}. "
            f"Loaded {len(good)} bases + ft_transformer. STOPPING."
        )
    print(f"BASELINE ASSERTED OK: {base_cv:.6f} ~ {n70_expected} (diff={diff:.6f} < 0.0002)")

    # ---- Load new candidates from a4_refresh_2026-06-12 ----
    A4 = COMP / "refs" / "a4_refresh_2026-06-12"

    new_candidate_defs = [
        ("stacking_0_97093",
         A4 / "stacking-0-97093" / "oof_lr_stacker_v7.npy",
         A4 / "stacking-0-97093" / "pred_lr_stacker_v7.npy"),
        ("meta_stacker_97105",
         A4 / "0-97105-sklearn-meta-stacker-2-5x-faster-saga" / "oof_lr_stacker_v7.npy",
         A4 / "0-97105-sklearn-meta-stacker-2-5x-faster-saga" / "pred_lr_stacker_v7.npy"),
        ("gpu_lr_v9",
         A4 / "gpu-logistic-regression-stacker" / "oof_lr_stacker_v9.npy",
         A4 / "gpu-logistic-regression-stacker" / "pred_lr_stacker_v9.npy"),
    ]

    candidates_oof = {}
    candidates_test = {}
    print("\n=== New candidate alignment + solo BA checks ===")
    for cname, oof_path, test_path in new_candidate_defs:
        try:
            raw_oof  = rd(oof_path, n)
            raw_test = rd(test_path, nt)
            if raw_oof.shape != (n, 3) or raw_test.shape != (nt, 3):
                print(f"  {cname}: VOID shape mismatch oof={raw_oof.shape} test={raw_test.shape}")
                continue
            oof_n = norm(raw_oof)
            solo_ba = score_fn(y, oof_n.argmax(1))
            print(f"  {cname}: solo_BA={solo_ba:.6f}  oof={raw_oof.shape}  test={raw_test.shape}")
            if solo_ba <= 0.85:
                print(f"  {cname}: VOID solo BA too low — column order issue")
                continue
            if solo_ba >= 0.972:
                print(f"  {cname}: QUARANTINE solo BA >= 0.972 — skipping")
                continue
            candidates_oof[cname]  = logp(oof_n)
            candidates_test[cname] = logp(norm(raw_test))
        except Exception as e:
            print(f"  {cname}: FAIL {str(e)[:80]}")

    print(f"\nValid new candidates: {list(candidates_oof.keys())}")
    ...
    # ---- Step 2: Greedy forward selection onto bank+ft_transformer ----
    print("\n=== Step 2: Greedy forward selection onto n70 base ===")
    ...
    print(f"n70 baseline:      {base_cv:.6f}")
    ...
    print(f"\n=== FINAL SUMMARY ===")
    print(f"n70_baseline (asserted): {base_cv:.6f}")
    print(f"new_candidates_found: {list(candidates_oof.keys())}")
    print(f"selected_bases: {selected}")
    print(f"per_fold_scores: {per_fold_scores}")
    print(f"cv={cv_mean:.6f}  sem={cv_sem:.6f}")
    print(f"lift_vs_champion={lift:+.6f}  beats_champion_by_2sem={beats}")
