...
def main():
    ...
    # RUN LGBM TIGHT arm
    print("\n" + "="*70, flush=True)
    print("=== LightGBM meta: TIGHT arm ===", flush=True)
    (oof_lgbm_tight, test_lgbm_tight, pf_lgbm_tight,
     cv_lgbm_tight, sem_lgbm_tight) = lgbm_meta_arm(
        OOF_tight, TST_tight, y, fval, "TIGHT"
    )

    # RUN LGBM FULL arm
    print("\n" + "="*70, flush=True)
    print("=== LightGBM meta: FULL arm ===", flush=True)
    (oof_lgbm_full, test_lgbm_full, pf_lgbm_full,
     cv_lgbm_full, sem_lgbm_full) = lgbm_meta_arm(
        OOF_full_arm, TST_full_arm, y, fval, "FULL"
    )

    # =========================================================================
    # Determine winning LGBM arm
    print("\n" + "="*70, flush=True)
    print("=== LGBM ARM COMPARISON ===", flush=True)
    print(f"TIGHT: cv={cv_lgbm_tight:.6f}  sem={sem_lgbm_tight:.6f}  per_fold={[f'{s:.6f}' for s in pf_lgbm_tight]}", flush=True)
    print(f"FULL:  cv={cv_lgbm_full:.6f}  sem={sem_lgbm_full:.6f}  per_fold={[f'{s:.6f}' for s in pf_lgbm_full]}", flush=True)

    if cv_lgbm_tight >= cv_lgbm_full:
        winner = "TIGHT"
        cv_win, sem_win, pf_win = cv_lgbm_tight, sem_lgbm_tight, pf_lgbm_tight
        oof_win, test_win = oof_lgbm_tight, test_lgbm_tight
    else:
        winner = "FULL"
        cv_win, sem_win, pf_win = cv_lgbm_full, sem_lgbm_full, pf_lgbm_full
        oof_win, test_win = oof_lgbm_full, test_lgbm_full

    print(f"\nLGBM WINNER: {winner}  cv={cv_win:.6f}  sem={sem_win:.6f}", flush=True)
