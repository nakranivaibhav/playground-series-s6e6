"""node_0108 — TabM on arcsinh-flux (luptitude / fs_luptitude) FE.

THE ONE ATOMIC CHANGE vs node_0107:
  Replace fs_flux_rich with fs_luptitude (arcsinh-flux, Lupton 1999).
  Training loop (TabM + PLR, k=32) is byte-identical to node_0033/node_0107.

fs_luptitude features (stateless):
  mu_b = arcsinh( f_b / (2 * SOFTENING) )
    where f_b = 10^(-0.4 * (mag_b - mag_mean))
    SOFTENING = 0.01  (fixed constant, NOT fit on data — keeps this stateless)
  5  luptitudes mu_b  (one per band)
  10 pairwise luptitude differences (mu_b - mu_b')
  2  luptitude aggregates: mean, range
  1  raw redshift
  1  log1p(redshift)
  Total numeric: 5+10+2+1+1 = 19 features
...
# TabM hyperparameters (byte-identical to node_0033/node_0107)
D_EMB = 16
N_BINS = 48
K_ENS = 32
DROPOUT = 0.1
...
# fs_luptitude constants
MAG_BANDS = ["u", "g", "r", "i", "z"]
BAND_PAIRS = [(a, b) for i, a in enumerate(MAG_BANDS) for b in MAG_BANDS[i + 1:]]  # 10 pairs
SOFTENING = 0.01  # fixed constant, NOT fit on data — keeps fs_luptitude stateless
...
# ─── fs_luptitude stateless FE (THE ONE CHANGE) ──────────────────────────────

def add_luptitude_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Pure row-wise stateless luptitude (arcsinh-flux) features (fs_luptitude).
    No fit, no target, no cross-row stats.  SOFTENING is a fixed constant.

    Numeric features (19 total):
      - 5  luptitudes mu_b = arcsinh(f_b / (2*SOFTENING))
           with f_b = 10^(-0.4*(mag_b - mag_mean))
      - 10 pairwise luptitude differences (mu_b - mu_b')
      - 2  luptitude aggregates: mean, range
      - 1  raw redshift
      - 1  log1p(redshift)
    Categorical features (kept from raw df):
    ...
    # Linear fluxes (mag-mean-centered so scale is ~1 near mean)
    fluxes = np.power(10.0, -0.4 * rel_mags).astype(np.float32)  # (N, 5)

    # Luptitudes: mu_b = arcsinh(f_b / (2*SOFTENING))
    lups = np.arcsinh(fluxes / (2.0 * SOFTENING)).astype(np.float32)  # (N, 5)

    result = {}

    # 5 luptitudes
    for j, b in enumerate(MAG_BANDS):
        result[f"_lup_{b}"] = lups[:, j]

    # 10 pairwise luptitude differences
    for a, b in BAND_PAIRS:
        result[f"_lupdiff_{a}_{b}"] = (lups[:, MAG_BANDS.index(a)] - lups[:, MAG_BANDS.index(b)]).astype(np.float32)

    # 2 luptitude aggregates
    result["_lup_mean"] = lups.mean(axis=1).astype(np.float32)
    result["_lup_range"] = (lups.max(axis=1) - lups.min(axis=1)).astype(np.float32)

    # redshift
    redshift = df["redshift"].astype(np.float32).values
...
# ─── TabM training (byte-identical to node_0033/node_0107) ───────────────────
...
# ─── fs_luptitude stateless FE (computed once, safe) ─────────────────────────
log("Applying fs_luptitude stateless FE ...")
X_raw = train_raw.drop(columns=[IDC, TARGET])
X_test_raw = test_raw.drop(columns=[IDC])

X_lup = add_luptitude_features(X_raw)
X_test_lup = add_luptitude_features(X_test_raw)

num_feature_cols = [c for c in X_lup.columns if c not in BASE_CAT_COLS]
log(f"  X_lup={X_lup.shape}  X_test_lup={X_test_lup.shape}")
...
assert TARGET not in X_lup.columns, f"LEAK: {TARGET} in features"
assert IDC not in X_lup.columns, f"LEAK: {IDC} in features"
...
for c in num_feature_cols:
    x = X_lup[c].values[sample_idx].astype(np.float32)
...
# Check 4: add_luptitude_features is stateless (fixed SOFTENING constant, no .fit calls)
# SOFTENING = 0.01 is a module-level constant, NOT fit on data.
log(f"Pre-flight: fs_luptitude stateless (SOFTENING={SOFTENING}, no .fit) — OK")
# Categorical factorize maps are fit INSIDE the fold loop (fit_in_fold).
...
for fi in folds_list:
    ...
    X_tr_fold, X_val_fold, X_te_fold, cat_cards = fit_fold_categoricals(
        X_lup.iloc[tr_idx].reset_index(drop=True),
        X_lup.iloc[val_idx].reset_index(drop=True),
        X_test_lup.copy(),
    )
