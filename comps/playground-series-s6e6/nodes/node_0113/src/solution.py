...
# NCL hyperparameters
GAMMA_SWEEP = [0.1, 0.3]   # swept at fold-0 only
...
# Select best gamma by BA
GAMMA_BEST = max(sweep_results, key=lambda g: sweep_results[g][0])
best_ba_f0, best_ec_f0 = sweep_results[GAMMA_BEST]
log(f"\nBest gamma: {GAMMA_BEST}  BA={best_ba_f0:.6f}  err_corr={best_ec_f0:.4f}")

# ─── GATE CHECKS ─────────────────────────────────────────────────────────────
log("\n=== GATE CHECKS ===")
gate1_pass = best_ba_f0 >= 0.965
gate2_pass = best_ec_f0 < 0.65
log(f"  Gate 1 (fold-0 BA >= 0.965): {gate1_pass}  BA={best_ba_f0:.6f}")
log(f"  Gate 2 (fold-0 err-corr < 0.65): {gate2_pass}  err_corr={best_ec_f0:.4f}")

if not gate1_pass:
    log(f"GATE 1 FAILED: fold-0 BA {best_ba_f0:.6f} < 0.965. STOPPING.")
    print(f"GATE_FAIL gate=1 BA={best_ba_f0:.6f} threshold=0.965", flush=True)
    # Write partial node.md gate info
    sys.exit(1)

if not gate2_pass:
    log(f"GATE 2 FAILED: fold-0 err-corr {best_ec_f0:.4f} >= 0.65. STOPPING.")
    print(f"GATE_FAIL gate=2 err_corr={best_ec_f0:.4f} threshold=0.65", flush=True)
    sys.exit(1)

log("Both gates passed. Proceeding to full 5-fold run.")
