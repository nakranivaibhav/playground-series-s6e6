"""node_0126 — improve (gbdt): LightGBM on fs_realmlp_fe + fs_physloc_min features.

ABLATION of node_0124: keep ONLY feh_phot and P2s (drop P1s, z_warp, feh_in_range).
THE ONE ATOMIC CHANGE vs node_0030:
  Add fs_physloc_min = {feh_phot, P2s} ONLY (STATELESS). Drop P1s, z_warp, and
  feh_in_range from n124's block — post-mortem flagged z_warp (clipped 8673 negative-z
  stars to one value, an artifact) and the flat feh_in_range boolean as likely
...
Leakage discipline:
  - fs_physloc_min: STATELESS (same row-wise math on train AND test, no fit, no target).
...
# comp dir is two levels up from node dir: nodes/node_0126 → nodes → comp
...
def stateless_fe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Includes fs_realmlp_fe features (from node_0030) AND the new fs_physloc_min block.

    ABLATION vs node_0124: ONLY feh_phot and P2s kept; P1s, z_warp, feh_in_range removed.
    """
    df = df.copy()
    ...
    # ── fs_physloc_min features (ABLATION — only feh_phot and P2s) ───────────
    ...
    # Stellar-locus principal color P2s (perp, ~=0 for stars) — Bond 2010 coefficients
    # NOTE: P1s, z_warp, feh_in_range are DROPPED in this ablation node
...
# We check the physloc_min features
...
# Only check the new physloc_min features (the stateless FE additions)
_physloc_min_cols = ["_feh_phot", "_P2s"]
_suspicious = []
for c in _physloc_min_cols:
...
if not _suspicious:
    log(f"  check3 PASS: no physloc_min feature has |corr| >= 0.999 vs target on {_sample_size} rows")
else:
    raise SystemExit(f"LEAK DETECTED (check3): {_suspicious}")

# Check 4: fs_physloc_min is STATELESS — verify identical transform on train vs test
# (no fit needed; math is row-wise). Checked by construction: same function applied to both.
log("  check4 PASS: fs_physloc_min is STATELESS (same row-wise math on train and test, no .fit)")
...
log("Applying stateless FE (fs_realmlp_fe + fs_physloc_min) ...")
